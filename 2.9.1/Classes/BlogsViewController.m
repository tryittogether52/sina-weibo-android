...
- (void)viewDidLoad {
    ...
    self.navigationItem.backBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Blogs", @"") style:UIBarButtonItemStyleBordered target:nil action:nil] autorelease];
...
- (void)postDidUploadSuccessfully:(NSNotification *)notification {
    ...
    uploadController.label.textColor = [UIColor colorWithRed:0.0f green:128.0f/255.0f blue:0.0f alpha:1.0f];
    uploadController.label.frame = CGRectMake(uploadController.label.frame.origin.x, uploadController.label.frame.origin.y - 12, uploadController.label.frame.size.width, uploadController.label.frame.size.height);
    [UIView commitAnimations];
