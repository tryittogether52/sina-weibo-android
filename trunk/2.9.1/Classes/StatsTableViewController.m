	...
	if ([xArray count] > 0){
		...
		if ([reportType isEqualToString:@"chartDaysData"] || [reportType isEqualToString:@"chartWeeksData"] || [reportType isEqualToString:@"chartMonthsData"]){
			...
			if ([reportType isEqualToString:@"viewsData"]){
                self.viewsData = [NSArray arrayWithArray:self.statsTableData];
				[self.tableView reloadData];		
			}
			if ([reportType isEqualToString:@"postViewsData"]){
                self.postViewsData = [NSArray arrayWithArray:self.statsTableData];
				[self.tableView reloadData];		
			}
			if ([reportType isEqualToString:@"referrersData"]){
                self.referrersData = [NSArray arrayWithArray:self.statsTableData];
				[self.tableView reloadData];		
			}
			if ([reportType isEqualToString:@"searchTermsData"]){
                self.searchTermsData = [NSArray arrayWithArray:self.statsTableData];
				[self.tableView reloadData];		
			}
			if ([reportType isEqualToString:@"clicksData"]){
                self.clicksData = [NSArray arrayWithArray:self.statsTableData];
				[self.tableView reloadData];		
			}
		}
	}
...
-(void) showNoDataFoundError{
	...
	UILabel *errorMsg = [[[UILabel alloc] init] autorelease];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	//if (cell == nil) {
		...
		if (addLoadMoreFooter){
            ...
            label.textColor = [UIColor colorWithRed:40.0 / 255 green:82.0 / 255 blue:137.0 / 255 alpha:1.0];
		}
		else {
			[cell addColumn:210];
...
- (void)viewUrl:(NSString*) urlString{
	...
	if (url != nil) {
        ...
        if (DeviceIsPad()) {
            nibName = @"WPWebViewController-iPad";
        }
        else {
            nibName = @"WPWebViewController";
        }

        WPWebViewController *webViewController = [[[WPWebViewController alloc] initWithNibName:nibName bundle:nil] autorelease];
